...
Feature: Medical Center Test scenarios
  ...
  Scenario: Patient Create and delete an appointment
    ...
    Then element with xpath "//p[contains(text(),'JaziD first appointment')]/ancestor::article/div[1]/h3" should contain text "Tuesday 18 February 2025, 09:00"
    ...
    And I wait for 1 sec
    And element with xpath "//p[contains(text(),'JaziD first appointment')]" should not be present
  ...
  Scenario: Patient Create and Cancel the delete for an appointment
    ...
    And I click on element with xpath "//button[contains(text(),'Save')]"
    #patient verify that this appointment is created
    Then I wait for element with xpath "//p[contains(text(),'JaziD first appointment')]" to be present
    ...
    #delete this appointment
    Then I wait for element with xpath "//p[contains(text(),'JaziD first appointment')]/ancestor::article/div[2]/span" to be present
    And I click on element with xpath "//p[contains(text(),'JaziD first appointment')]/ancestor::article/div[2]/span"
    #confirmation pop window
    Then I wait for element with xpath "//h2[contains(text(),'Are you sure you want to cancel appointment?')]" to be present
